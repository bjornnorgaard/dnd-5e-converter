...
func TestSpellToMarkdown(t *testing.T) {
	...
	parser := Parser{}
	md, err := parser.spellToMarkdown(spell)
	if err != nil {
...
func TestParseSpells_WithMockData(t *testing.T) {
	...
	// Create a parser with the test directories
	p := New(Config{
	...
	// Parse the spells
	if err := p.ParseSpells(context.Background()); err != nil {
...
func TestSpellToMarkdown_EdgeCases(t *testing.T) {
			...
			spell: Spell{
				...
				Entries: []interface{}{
					...
					map[string]interface{}{
						...
						"rows": []interface{}{
							[]interface{}{"1-2", "Failure"},
							[]interface{}{"3-4", "Partial success"},
							[]interface{}{"5-6", "Success"},
						},
					},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			md, err := parser.spellToMarkdown(tt.spell)
			if err != nil {
				t.Fatalf("spellToMarkdown() error = %v", err)
			}
