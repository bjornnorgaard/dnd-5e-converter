...
func (p Parser) ParseSpells(ctx context.Context) error {
	var (
		spellsPath = filepath.Join(p.DataDirectory, "spells")
		indexPath  = filepath.Join(spellsPath, "index.json")
	)

	// Create output directory if it doesn't exist
	outDir := filepath.Join(p.OutDirectory, "spells")
	...
	for source, filename := range index {
		if err := p.processSpellFile(ctx, spellsPath, outDir, source, filename); err != nil {
...
func (p Parser) processSpellFile(ctx context.Context, spellsPath, outDir, source, filename string) error {
	...
	for _, spell := range spellFile.Spell {
		mdContent, err := p.spellToMarkdown(spell)
		if err != nil {
...
func (p Parser) spellToMarkdown(spell Spell) (string, error) {
